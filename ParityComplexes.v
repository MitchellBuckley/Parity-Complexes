...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  ...
  Proof with intuition.
   ...
   refine (strong_induction _ _ _)...
     ...
     - assert ((x = z) \/ ~(x = z))...
         ...
         left; subst; left...
         admit.
       assert ((less z x) \/ ~(less z x))...
         apply less_decidable.
         left.
         admit.
       set (R := fun r => triangle_rest (Setminus T (Singleton x)) z r /\ ~(r = z)).
       set (Q := fun r => triangle_rest (Setminus T (Singleton z)) r x /\ ~(r = x)).
       assert (Finite R) as RFin. 
         apply (Finite_Included'' T)...
         apply Cardinal_are_Finite in TCard...
         unfold R, Included, In at 1...
         apply triangle_rest_in_set in H3...
         basic...
         unfold R, In...
         assert ((x0 = z) \/ ~(x0 = z))...
           apply carrier_decidable_eq.
         assert ((triangle_rest (Setminus T (Singleton x)) z x0) \/ ~(triangle_rest (Setminus T (Singleton x)) z x0))...
           apply (H n)...
           apply (Cardinal_Setminus carrier_decidable_eq (S n))... admit.
       assert (Finite Q) as QFin. 
         ...
         assert ((x0 = x) \/ ~(x0 = x))...
           apply carrier_decidable_eq.
         assert ((triangle_rest (Setminus T (Singleton z)) x0 x) \/ ~(triangle_rest (Setminus T (Singleton z)) x0 x))...
           apply (H n)...
           apply (Cardinal_Setminus carrier_decidable_eq (S n))... admit.
       assert (Finite (Intersection R Q)) as RQFin.
         apply Finite_Intersection...
       apply Finite_Empty_or_Inhabited in RQFin as [RQcond | RQcond]... 
       + right...
         apply triangle_rest_equiv' in H0.
         rename H0 into K.
         inversion K...
         assert (In Empty_set y)...
         rewrite <- RQcond; apply In_Intersection; split; unfold R, Q, In.
         * intuition. apply triangle_rest_equiv'.
           admit. admit.
         * admit.
       + left...
         inversion RQcond; clear RQcond.
         rename x0 into y.
         apply triangle_rest_equiv'.
         unfold R, Q in H0. 
         apply In_Intersection in H0.
         unfold In in H0...
         apply tr_trans'' with y; apply triangle_rest_equiv'. 
         apply (triangle_rest_Included (Setminus T (Singleton x))); 
         [assumption | apply Setminus_Included].
         apply (triangle_rest_Included (Setminus T (Singleton z))); 
         [assumption | apply Setminus_Included].
  Qed.

  Inductive is_a_path : list carrier -> Prop :=
    | fff'' : forall x, is_a_path (cons x nil)
    | ffff'' : forall x y, less x y -> forall l, is_a_path (cons y l) -> is_a_path (cons y (cons x l)).
  Inductive is_in_set (M : Ensemble carrier) : list carrier -> Prop :=
    | ggg'' : forall x, In M x -> forall k, is_in_set M k -> is_in_set M (cons x k).
  Inductive starts (x : carrier) : list carrier -> Prop := 
    | iii'' : forall l, starts x (cons x l).
  Inductive ends (x : carrier) : list carrier -> Prop := 
    | iiii'' : forall l, starts x (List.rev l) -> ends x l.
  Definition no_loop (l : list carrier) : Prop := List.NoDup l.
  Definition dec_Prop (P : Prop) := P \/ ~P.
  Lemma FFFF : forall A B, dec_Prop A /\ dec_Prop B -> dec_Prop (A /\ B).
  Proof with intuition.
    unfold dec_Prop...
  Qed.
  ...
  Proof with intuition.
    unfold dec_Prop...
    destruct l.
    - right; intros G; inversion G.
    - induction l.
      + left; left.
      + inversion IHl; clear IHl.
        * assert ((less a c) \/ ~(less a c))...
            apply less_decidable.
          left; right...
          right; intros G; inversion G...
        * right; intros G; inversion G...
  Qed.
  Lemma is_in_set_dec : forall M l, decidable M -> dec_Prop (is_in_set M l).
  Proof with intuition.
    unfold dec_Prop...
    rename H into Mdec.
    induction l. 
    ...
    - intuition.
      + unfold decidable in Mdec. 
        specialize Mdec with a. 
        inversion Mdec; clear Mdec.
        * left; constructor...
        * right; intros G; inversion G...
      + right; intros G; inversion G...
  Qed.
  Definition starts_and_ends (x z : carrier) (l : list carrier) : Prop := starts x l /\ ends z l. 
  Lemma starts_dec : forall l x, dec_Prop (starts x l).
  Proof with intuition. 
    unfold dec_Prop, starts_and_ends. 
    intros.
    destruct l.
    - right... inversion H... 
    - assert ((x = c) \/ ~(x = c)).
        apply carrier_decidable_eq.
      inversion H; clear H; [subst | right]...
      + left... constructor.
      + inversion H...
  Qed. 
  Lemma ends_dec : forall l x, dec_Prop (ends x l).
  Proof with intuition. 
    unfold dec_Prop, starts_and_ends. 
    intros.
    ...
    - assert ((x = c) \/ ~(x = c)).
      ...
      + constructor.
        subst. 
        rewrite (List.rev_involutive). constructor.
      + inversion H.  
        rewrite (List.rev_involutive) in H1. 
        inversion H1... 
  Qed. 
  Lemma no_loop_dec : forall l, dec_Prop (no_loop l).
  Proof with intuition.
    unfold dec_Prop...
    induction l. 
    - left; left. 
    - intuition.
      + assert ((List.In a l) \/ ~(List.In a l)).
          clear H. 
          induction l. 
          right... 
          intuition.
  ...
  Lemma loops_and_length : forall n, forall M, Cardinal M (n) -> 
       forall l, is_in_set M l -> (no_loop l) -> (length l <= n).
  Proof with intuition.
    intuition.
  admit.
  Qed.
  Definition asdf (M : Ensemble carrier) (n : nat) := fun w => ((no_loop w) /\ (is_in_set M w) /\ @length carrier w <= n).
  Lemma Union_asdf : forall M n, asdf M (S n) == Union (asdf M n) (fun w => ((no_loop w) /\ (is_in_set M w) /\ @length carrier w = S n)).
  Admitted.
  Lemma list_le_n_Finite : 
    forall n, 
    (forall M, 
    @Cardinal carrier M (n) ->
    Finite (asdf M (n))).
  Proof with intuition.
    admit.
  Qed.
  ...
  Proof with intuition.
    ...
    inversion H as [n K]; clear H.
    apply (Finite_Included'' (asdf M n))... 
    + apply (list_le_n_Finite n M)...
    + unfold Included, In, asdf... 
      apply (loops_and_length n M)...
    + unfold In... 
      unfold starts_and_ends... 
      apply FFFF... apply is_a_path_dec. 
      apply FFFF... apply is_in_set_dec. apply all_decidable. apply Cardinal_are_Finite in K... 
      apply FFFF... apply no_loop_dec. 
      apply FFFF... apply starts_dec.
      apply ends_dec.
  Qed.
  ...
  Lemma equiv''' : 
    forall T, 
      ...
      forall x z, (triangle_rest T z x <-> (exists l, 
        ...
        (no_loop l) /\ starts_and_ends z x l)).
  Proof with intuition.
    intros T TFin.
    intuition. 
    - induction H... 
      ...
      + inversion IHtriangle_rest; clear IHtriangle_rest...
        exists (cons x x0)...
        inversion H3... 
        admit. admit.
        constructor...
  ...
  Proof with intuition.
    ...
    assert (Finite (fun l =>
                ...
                is_in_set T l /\
                (no_loop l) /\
                starts_and_ends z x l)).
      apply loop_free_path_dec...
    apply Finite_Empty_or_Inhabited in H0...
    - right...
      apply equiv''' in H0...
      inversion H0; clear H0.
      assert (In Empty_set x0)... 
      rewrite <- H1. unfold In...
    - left...
      apply equiv'''...
      inversion H1. exists x0. unfold In in H0. assumption.
  Qed. 
      ...
      + unfold N, Q.
        ...
        * rewrite <- Setminus_is_Intersection_Complement.
          ...
          assert (S (dim y0) <= m)...
            ...
            assert (S (dim x0) = dim x)...
            rewrite <- H7, H10, dimx in H6...
          inversion H9; clear H9; subst...
          repeat (basic; intuition).
          assert (dim x0 = m)...
            rewrite H8, dimx in H7...
          assert (S (dim x0) = dim y0)...
            rewrite H7 in H8... 
          assert (S (dim x0) <= m)...
            rewrite H7, dimx in H8...
        * rewrite Union_sym.
          apply Finite_Union...
