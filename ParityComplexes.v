...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  ...
  Lemma wf_minimal_exists :
    forall X n, Finite X /\ Inhabited X /\ (X ⊆ sub Full_set (S (S n))) ->
      well_formed X ->
        exists x, In X x /\ Disjoint (minus x) (Plus X).
  Proof with intuition.
    idtac...
...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  ...
  Proof with intuition.
    ...
    assert (exists x, In X x /\ Disjoint (minus x) (Plus X)) as R. 
    ...
    assert (is_a_cell ((sup N n  ∪ Y'), (sup Q n ∪ Y')) /\ Plus Z ∩ sub Q (S n) == Empty_set) as AA.
      ...
      apply (IHm Z)...
        ...
        assert (sub Q (S n) == Intersection (Union (sub P (S n)) (plus x)) (Complement (minus x))) as JA.
            ...
            rewrite Plus_Singleton.
            rewrite Minus_Singleton...
        assert (MinusPlus Z ⊆ Intersection (Union (MinusPlus X) (plus x)) (Complement (minus x))) as JB.
          unfold MinusPlus.
          rewrite XZrel.
          rewrite Plus_Union.
          rewrite Minus_Union.
  ...
  Qed.
    ...
    Proof with intuition.
      ...
      + constructor...
        ...
        assert (plus x moves A' to B') as Q1.
          ...
          rewrite Plus_Singleton...
          apply well_formed_sub; apply MPcell...
      assert (is_a_cell (Union (sup M m) A', Union (sup P m) A')) as Q4.
        unfold A'.
    ...
    Proof with intuition.
      ...
      assert (is_a_cell (Union (sup M m) A', Union (sup P m) A')) as Q4.
        ...
        unfold AP, AM.
          ...
          rewrite <- Setminus_is_Intersection_Complement.
          rewrite <- Adim...
      assert (is_a_cell (Union (sup M m) B', Union (sup P m) B')) as Q6.
        unfold B'. 
    ...
    Proof with intuition.
      ...
      + constructor...
        ...
        inversion H1 as [w [R1 R2]]; clear H1...
        inversion H2 as [v [R3 R4]]; clear H2...
        set (Z := fun y => In T y /\ triangle_rest T y w).
        assert (Included Z T) as ZinT.
          unfold Z, Included, In at 1...
        assert (exists m : carrier, m ∈ Z /\
        ...
        assert (plus x moves A' to B') as Q1.
          ...
          constructor...
            apply In_Intersection in H1...
             ...
             apply Disjoint_sym...
      assert (is_a_cell (Union (sup M m) B', Union (sup P m) B')) as Q4.
        unfold B'.
        set (BM := sup M m ∪ sub B (1+m)).
        set (BP := sup P m ∪ sub B (1+m)).
        assert (is_a_cell (BM, BP)).
        ...
        unfold BP.
          ...
          rewrite <- Setminus_is_Intersection_Complement.
          rewrite <- Bdim...
      assert (is_a_cell (Union (sup M m) A', Union (sup P m) A')) as Q6.
        unfold A'. 
        set (AM := sup M m ∪ sub A (1+m)).
        set (AP := sup P m ∪ sub A (1+m)).
        assert (is_a_cell (AM, AP)).
...
(B)) (S n) ∪ Minus (plus x)) ∩ √Plus (plus x)) ∪ (plus x))
    ...
    assert ((minus x) moves A to B) as moves4.
      apply Prop_3_1.
      assumption.
      assert (receptive ((sup M n ∪ A) ∪ plus x)
           ∧ receptive ((sup P n ∪ B) ∪ plus x)).
      apply Prop_3_3...
      unfold celldim; rewrite <- Adim, <- Bdim; unfold setdim; repeat (basic; intuition).
        assert (dim x0 = n)... rewrite H4... 
        assert (S (dim x0) = dim x)... 
        assert (dim x0 = S n)... rewrite H8...
        assert (dim x0 = n)... rewrite H4... 
        assert (S (dim x0) = dim x)... 
        assert (dim x0 = S n)... rewrite H8...
      rewrite <- Adim.
      assert (receptive (sub ((sup (M) n ∪ A) ∪ plus x) (S n))).
      ...
      assert (sub (plus x) (S n) == Empty_set).
        ...
        rewrite xdim in H...
      rewrite H in H0.
      rewrite Empty_set_ident_left, Empty_set_ident_right in H0...
...
(B)) (S n) ∪ Minus (minus x)) ∩ √Plus (minus x)) ∪ (minus x))
    ...
    assert ((plus x) moves A to B) as moves4.
      apply Prop_3_1_dual.
      assumption.
      assert (receptive ((sup M n ∪ A) ∪ minus x)
           ∧ receptive ((sup P n ∪ B) ∪ minus x)).
      apply Prop_3_3...
      unfold celldim; rewrite <- Adim, <- Bdim; unfold setdim; repeat (basic; intuition).
        assert (dim x0 = n)... rewrite H4... 
        assert (S (dim x0) = dim x)... 
        ...
        assert (dim x0 = S n)... rewrite H8...
      rewrite <- Bdim.
      assert (receptive (sub ((sup P n ∪ B) ∪ minus x) (S n))).
      apply receptive_by_dimension...
      repeat (rewrite sub_Union in H0).
      rewrite sub_sup_Empty_set in H0...
      assert (sub (minus x) (S n) == Empty_set).
        rewrite <- Minus_Singleton.
        rewrite sub_Minus.
        rewrite sub_Singleton_Empty_set...
  ...
  Proof.
    ...
    + intros m; destruct m.
      ...
      - split. 
        ...
        apply IHn. apply dim_Prop_3_3 with (S n); auto.
        apply IHn. apply dim_Prop_3_3 with (S n); auto.
        assumption.
        apply Lemma_3_2_dual_Step_2.
  ...
  Lemma Lemma_3_2_b :
    forall n, Prop_3_3_st n -> forall m, (Lemma_3_2_b_st n m).
  Proof.
    intros n Prop_3_3.
  ...
  Lemma Lemma_3_2_b_dual :
    forall n, Prop_3_3_st n -> forall m, (Lemma_3_2_b_dual_st n m).
  Proof.
    intros Prop_3_3 n.
    apply Lemma_3_2_b_work...
    assumption.
  ...
  Lemma Lemma_3_2_c :
    forall n, Prop_3_3_st n -> forall m, (Lemma_3_2_c_st n m).
  Proof.
    intros.
  ...
  Lemma Lemma_3_2_c_dual :
    forall n, Prop_3_3_st n -> forall m, (Lemma_3_2_c_dual_st n m).
  Proof.
    intros.
...
(* ~~~~~~~~~~~~   Prop_3_3  ~~~~~~~~~~~~~~~ *)
  ...
  Definition n_receptive : nat -> Prop := 
    fun n => forall M P, is_a_cell (M, P) -> 
    celldim (M, P) n -> forall x, dim x <= 2+n -> 
     receptive_x M x /\ receptive_x P x.

  Lemma n_receptive_zero : n_receptive 0.
  Proof with intuition.
    unfold n_receptive, receptive_x. 
    intros M P MPcell MPdim x xdim. 
    ...
    assert (P == sub P 1) as Pdim. 
      admit.
    rewrite Mdim, Pdim, <- K.
    inversion xdim as [xdim' | z xdim']; clear xdim.
    ...
    + assert (∃ d : carrier, sub M 1 == Singleton d). 
     ...
     + clear H0 z. 
       ...
       rewrite H; intuition;
       rewrite Empty_set_zero_left in H2; inversion H2...
       assert (plus x == Empty_set). admit.
       assert (minus x == Empty_set). admit.
       rewrite H0, H1.
       rewrite Plus_Empty_set, Minus_Empty_set.
       repeat (rewrite (Empty_set_zero_right)).
       intuition.
  Qed.

  Lemma n_receptive_ind : forall n, (forall m, m <= n -> n_receptive m) -> (forall m, Lemma_3_2_b_st (S n) m) -> (forall m, Lemma_3_2_b_dual_st (S n) m) -> n_receptive (S n).
  Proof with intuition.
    intros n H L1 L2.
    unfold n_receptive...
    + inversion H2; clear H2. 
      ...
      - subst. 
        ...
        assert (is_a_cell (source n (M, P))).
          apply source_is_a_cell with n...
        unfold source in H2.
        apply (H n) with (x:=x) in H2...
        Focus 2. apply source_dim...
        apply receptive_x_by_dimension'; intros k. 
        assert ({k <= S n} + {S n < k})...
        * apply receptive_x_by_dimension with (n:=k) in H3.
          rewrite sub_sup_cancel in H3...
        * unfold receptive_x... 
          assert (S (k) = dim x)...
          inversion H7; clear H7.
          apply In_Intersection in H8...
...
Lemma Lemma_3_2_b_dual_corr : forall n, Prop_3_3_st n -> 
                          forall x, forall M P,
...
Proof with intuition.
  intros n Prop_3_3...
  pose (Lemma_3_2_b_dual n Prop_3_3 1).
...
Lemma Lemma_3_2_b_corr : forall n, Prop_3_3_st n -> forall x, forall M P,
                          is_a_cell (M, P) /\ celldim (M, P) n ->
                          dim x = (S n) /\ plus x ⊆ sub M (S n) ->
                          well_formed ((sub P (S n) ∪ minus x) ∩ √plus x).
Proof with intuition.
  intros n Prop_3_3...
  pose (Lemma_3_2_b n Prop_3_3 1 (Singleton x)).
  assert (is_a_cell
            (sup M n
...
Qed.
  ...
  Proof with intuition.
    intros n Prop_3_3 M P cellcond dimcond nsize u udim notatomic.
...
(*
  Sketch of Lemma_4_1
    ...
    Given a cell M P of dimension 2+n.
    let u be an element of sub M 2+n

    set (Z := (fun m => (m < n) /\ ~((sub M (S (S m)), sub P (S (S m))) === (sub (mu u) (S (S m)), sub (pi u) (S (S m)))))).
    let m be maximum in Z.

    let w be an element of ((sub M (S (S m))) ∩ (sub P (S (S m))))

    set (X := fun k => triangle_rest (sub M (S (S m))) k w).
    let x be a minimum of X.

    set (Y := fun k => triangle_rest (sub M (S (S m))) w k).
    let y be a maximum of Y.

    if (x ∈ ((sub M (S (S m))) ∩ (sub P (S (S m))))) then
    ...
    else (y ∈ ((sub M (S (S m))) ∩ (sub P (S (S m)))))) and
    ...
    carrier :=  list nat
    dim := length
    face : nat -> list nat -> list nat
     plus := fun l => (fun l' => exists m, even m /\ l' = face m l)
    minus := fun l => (fun l' => exists m,  odd m /\ l' = face m l)

  Lemma Lemma_3_3_work : 
    forall n, (forall k, k <= n -> (forall m, (Lemma_3_2_b_dual_st k m)) /\ (forall m, (Lemma_3_2_b_st k m))) -> Prop_3_3_st (S n).
  Proof with intuition.
    induction n. 
    - admit. 
      (* unfold Prop_3_3_st...
        unfold receptive.
        assert (exists d, M == Singleton d). admit. *)
    - unfold Prop_3_3_st...
      + unfold receptive...
        * inversion H4; clear H4. 
          apply In_Intersection in H5...
          ...
          assert (S (S (dim x0)) = dim x).
            ...
            assert (S (dim x1) = dim x)...
            rewrite <- H7...
          assert (dim x <= S (S (S (S n)))).
            rewrite <- H7...
          inversion H8; clear H8...
          admit. admit.
          (* *)
          assert (Minus (minus x) ∩ sub M (S (S (n))) == Empty_set).
            unfold Lemma_3_2_b_st in H.
            eapply H with (m := 1) (P:=P)... 
            admit.
            
            assert (Finite (minus x))...
            apply Cardinality_exists in H8...
            inversion H8... 
            eapply Hyp2 with (m := x1) (P := P)...
            unfold Included, sub, In at 2...
            admit. admit. assert ()
            apply H.
            assert (Finite (minus x))...
            apply Cardinality_exists in H8...
            inversion H8... apply H9. exists x1. act H9.
              apply
            admit.
