...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
 ...
 Lemma cell_dim_n_property : 
  ...
  Proof with intuition. 
    ...
    assert (exists m, In (sub P (S n)) m /\ (forall y, In (sub P (S n)) y -> (triangle_rest (sub P (S n)) m y) -> (m = y))).
      refine (maximal_exists' _ _ (S n) _)...
      refine (Finite_sub P _ (S n)). 
      apply H.
    
    inversion H1 as [w J]; clear H1...
    
    assert ( PlusMinus (sub P (S n)) ⊆ sub P n). 
    ...
    assert (Finite (sub P n)). 
      unfold is_a_cell in H...
    apply Finite_Empty_or_Inhabited in H4...
    exfalso. 
    assert (PlusMinus (sub P (S n)) == Empty_set). 
      apply (Included_Empty_set _ (sub P n))...
    assert (forall x, In (Plus (sub P (S n))) x -> (In (Minus (sub P (S n)))) x)...
      assert (x ∈ Minus (sub P (S n)) \/ ~(x ∈ Minus (sub P (S n))))... 
        apply all_decidable. 
        apply Minus_Finite. apply Finite_sub. apply H.
      exfalso. 
      assert (In (PlusMinus (sub P (S n))) x); unfold PlusMinus...
      rewrite H4 in H7...
  ...
  Proof with intuition.
    ...
    destruct n.
    - apply (Same_set_is_a_cell (sub P 0) (sub P 0))... 
        apply P_0_Inhabited in MPcell. 
        inversion MPcell; clear MPcell. 
      apply (Same_set_is_a_cell (Singleton x) (Singleton x))... 
      assert (dim x = 0) as dimx.
        assert (In (sub P 0) x)... rewrite H...
      unfold is_a_cell... 
        exists x...
        exists x...
    ...
    assert (sup M (0 - 1) == Empty_set) as AA. admit. (* coding issue *)
    rewrite AA. 
    rewrite Empty_set_ident_right...
  ...
  + unfold moves_def in *...
    * assert (forall k, sub (sup P (S n)) k == sub (((sub P (S n) ∪ sup M n) ∪ Plus (sub P (S n) ∪ sup M n)) ∩ √Minus (sub P (S n) ∪ sup M n)) k).
        intros.
        rewrite <- Setminus_is_Intersection_Complement. 
        ...
        assert ({k < n} + {k = n} + {k = S n} + {S n < k})... 
          ...
          apply sub_Same_set_Proper... 
          rewrite b.
          repeat (rewrite (sub_sup_cancel (S n) (S n)) || rewrite (sub_sup_Empty_set n) || 
          ...
          repeat (rewrite (sub_sup_Empty_set) || 
                  ...
                  rewrite Plus_Empty_set || 
                  rewrite Minus_Empty_set || rewrite Setminus_Empty_set)...
      apply Same_set_by_dimension in H7...
    * assert (forall k, sub (sub P (S n) ∪ sup M n) k == sub ((sup P (S n) ∪ Minus (sub P (S n) ∪ sup M n)) ∩ √Plus (sub P (S n) ∪ sup M n)) k).
        intros.
        rewrite <- Setminus_is_Intersection_Complement. 
        repeat (rewrite sub_Setminus || rewrite sub_Union || rewrite sub_Minus || rewrite sub_Plus)... 
        assert ({k < n} + {k = n} + {k = S n} + {S n < k})... 
  ...
  + unfold moves_def in *...
    * assert (forall k, sub (sup P (S n)) k == sub (((sub P (S n) ∪ sup M n) ∪ Plus (sup P (S n))) ∩ √Minus (sup P (S n))) k).
        intros.
        rewrite <- Setminus_is_Intersection_Complement. 
        repeat (rewrite sub_Setminus || rewrite sub_Union || rewrite sub_Minus || rewrite sub_Plus)... 
        assert ({k < n} + {k = n} + {k = S n} + {S n < k})... 
          ...
          apply sub_Same_set_Proper... 
          rewrite b.
          repeat (rewrite (sub_sup_cancel (S n) (S n)) || rewrite (sub_sup_Empty_set) || 
                  rewrite sub_idemp || rewrite (sub_sub_Empty_set) || 
                  rewrite Empty_set_ident_left || rewrite Empty_set_ident_right ||
                  rewrite Plus_Empty_set || 
                  rewrite Minus_Empty_set || rewrite Setminus_Empty_set)...
          repeat (rewrite (sub_sup_Empty_set) || 
                  ...
                  rewrite Minus_Empty_set || rewrite Setminus_Empty_set)...
      apply Same_set_by_dimension in H7...
    * assert (forall k, sub (sub P (S n) ∪ sup M n) k == sub ((sup P (S n) ∪ Minus (sup P (S n))) ∩ √Plus (sup P (S n))) k).
        intros.
        rewrite <- Setminus_is_Intersection_Complement. 
        repeat (rewrite sub_Setminus || rewrite sub_Union || rewrite sub_Minus || rewrite sub_Plus)... 
        assert ({k < n} + {k = n} + {k = S n} + {S n < k})... 
...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  ...
  Proof with intuition.
    ...
    assert (X moves Y to (sub M n)) as movement_result.
      unfold moves_def... 
      unfold Y. 
      rewrite U_I_dist_r. 
      ...
      rewrite Empty_set_ident_right.
      fold (PlusMinus X).
      assert (sub M n ∩ √Minus X == sub M n). 
        unfold Same_set, Included... 
    ...
    assert ((sub Y n) moves (sub M (n - 1)) to (sub P (n - 1))) as movement_result2.
      unfold is_a_cell in H2... 
      fold Y in H11.
      assert ((sub (sup M (n - 1) ∪ Y) n) moves (sub (sup M (n - 1) ∪ Y) (n - 1)) to (sub (sup P (n - 1) ∪ Y) (n-1))). 
        admit.  (* coding issue *)
      assert (sub Y (n - 1) == Empty_set). 
        admit. (* coding issue *)
      repeat (rewrite sub_Union in H12 || rewrite H14 in H12 ).
      (rewrite (sub_sup_cancel (n-1)) in H12).
      (rewrite (sub_sup_cancel (n-1)) in H12).
      (rewrite (sub_sup_Empty_set (n-1)) in H12).
    ...
    unfold is_a_cell in *...
    - apply (Inhabited_Included (sup M (n - 1) ∪ Y))... 
...
Lemma SSS : forall n, S (S n) <= n -1 -> False. Admitted.
Lemma TTT : forall n,   (S n) <= n -1 -> False. Admitted.
Lemma UUU : forall n,      n  <= n -1 -> False. Admitted.

    - unfold Y. 
      assert (X == sub X (S n)) as dimX.
      ...
      rewrite dimX.
      assert (∀ T M P : Ensemble carrier, T moves M to P -> (∀ n : nat, sub T (S n) moves sub M n to sub P n)) as DDD.
        apply moves_by_dim.
      assert ({S n0 < n} + {S n0 = n} + {S n0 = S n} + {S n < S n0 })...
        apply lt_eq_eq_lt_dec.
      ...
      + rewrite <- b. 
        ...
        rewrite b.
        fold Y. rewrite <- b... admit. (* coding issue, use movement_result2 *)
      + inversion b. 
        repeat (rewrite sub_idemp || rewrite (sub_sub_Empty_set) || 
                rewrite Empty_set_ident_right || 
                rewrite (sub_sup_Empty_set) || 
        ...
        rewrite <- (cell_dim_n_property M P)... unfold is_a_cell... 
      + repeat (rewrite (sub_sup_Empty_set) || 
                rewrite (sub_sub_Empty_set _ (S n0)) || 
                rewrite (sub_sub_Empty_set n (n0)) || 
        ...
        assert (sub P n0 == Empty_set). 
          unfold Same_set, Included... 
          unfold sub, In at 1 in H18... 
          assert (dim x <= n)... 
          rewrite H22 in H18.
          exfalso. unfold lt in b. assert (S n <= n)... apply (le_trans _ n0)... 
          inversion H18... 
    ...
    - unfold Y. assert (X == sub X (S n)) as dimX. admit.
      apply moves_by_dim; intros.
      repeat (rewrite <- Setminus_is_Intersection_Complement || rewrite sub_Setminus || rewrite sub_Union || rewrite sub_Minus || rewrite sub_Plus)... 
      rewrite dimX.
      assert (∀ T M P : Ensemble carrier, T moves M to P -> (∀ n : nat, sub T (S n) moves sub M n to sub P n)) as DDD.
      ...
      + inversion b. 
        ...
        rewrite <- (cell_dim_n_property M P)... unfold is_a_cell... 
      + repeat (rewrite (sub_sup_Empty_set) || 
                rewrite (sub_sub_Empty_set (S n) (S n0)) || 
                rewrite (sub_sub_Empty_set n (n0)) || 
                rewrite Empty_set_ident_right || 
                rewrite Plus_Empty_set || rewrite Empty_set_ident_left ||  
                rewrite Minus_Empty_set || rewrite Setminus_Empty_set)...
...
(*
  ...
  *)  (* I don't know what this is for, leave it until i'm sure the lemma works *)
    ...
    fold (is_a_cell'
        ((sup
            (sup (sup tM (n - 1) ∪ ((sub tM n ∪ Minus T) ∩ √Plus T)) (n - 1)
             ∪ ((sub (sup tM (n - 1) ∪ ((sub tM n ∪ Minus T) ∩ √Plus T)) n
                 ∪ Minus (plus x)) ∩ √Plus (plus x))) (n - 1)
          ∪ ((sub
                (sup (sup tM (n - 1) ∪ ((sub tM n ∪ Minus T) ∩ √Plus T))
                   (n - 1)
                 ∪ ((sub (sup tM (n - 1) ∪ ((sub tM n ∪ Minus T) ∩ √Plus T))
                       n ∪ Minus (plus x)) ∩ √Plus (plus x))) n ∪ Minus S')
             ∩ √Plus S')) ∪ S') (
        (sup (sup tP (n - 1) ∪ ((sub tM n ∪ Minus T) ∩ √Plus T)) (n - 1)
         ∪ ((sub (sup tM (n - 1) ∪ ((sub tM n ∪ Minus T) ∩ √Plus T)) n
...
(* this is tidying up, it is completely optional, doesn't change the meaning of anything, but might change
  ...
  Qed.
    ...
    assert ( Plus S' ∩ Plus (minus x) == Empty_set) as PerpS'minusxA.
      assert (Plus S' ∩ Plus (minus x) == Plus S' ∩ (Complement (Minus S')) ∩ Plus (minus x)).
        unfold Same_set, Included; repeat (basic; intuition)... 
        apply In_Complement... 
        assert (x0 ∈ (Minus (minus x) ∩ Plus (plus x))).
          admit. (* easy *) clear H12.
        apply In_Intersection in H13...
        assert (Disjoint (Plus (plus x)) (Plus S')).
          apply (bbbbb (sub M (S n)))...
          rewrite DisjUnion...
          apply well_formed_sub. apply MPcell.
        inversion H13. apply (H15 x0)...
      rewrite H.
      assert ((Plus S' ∩ √Minus S') ∩ Plus (minus x) ⊆ A ∩ Plus (minus x)).
        unfold A.
        unfold Included; repeat (basic; intuition)...
      assert (A ∩ Plus (minus x) ⊆ Empty_set).
        admit. (* ?? *)
      unfold Same_set...
    ...
    assert (well_formed (S' ∪ (minus x))) as WF1.
      ...
      assert (sub (S' ∪ minus x) 0 == Empty_set).
        unfold Same_set, Included... 
          exfalso.
    ...
    assert (well_formed ((minus x) ∪ T)) as WF2.
      ...
      assert (sub (minus x ∪ T) 0 == Empty_set).
        unfold Same_set, Included... 
          exfalso.
          unfold sub, In at 1 in H6... 
          apply In_Union in H7... 
    ...
    unfold Same_set, Included... 
      ...
      apply In_Union in H15...
        ...
        assert (well_formed (sub M (S n)))... apply well_formed_sub. apply MPcell. 
        assert (x2 = x3)... 
          unfold well_formed in H15...
          refine (H20 _ _ _ n _ _ _)...
          rewrite DisjUnion... assert (In (sub M (S n)) x2)... rewrite DisjUnion...
          unfold perp in H15... 
          assert (In Empty_set x1)...
            rewrite <- H21...
        apply (H9 x2); subst...
...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  ...
  Proof with intuition.
    ...
    assert (exists m, (In Z m /\ forall r, In Z r -> r <= m)) as max_exists.
      ...
      exfalso. 
      unfold Same_set, Included, Z in H...
      admit. (* ?? *)
    inversion max_exists as [m H]; clear max_exists; inversion H as [minZ mmax]; clear H. 
    
    assert (m < n) as mltn. 
      unfold Z, In at 1 in minZ... 
    assert (~((sub M (S m), sub P (S m)) === (sub (mu u) (S m), sub (pi u) (S m)))) as maxprop1.
      unfold Z, In at 1 in minZ...
    assert (forall r, S m < r ->  ((sub M r, sub P r) === (sub (mu u) r, sub (pi u) r))) as maxprop2.
      (* fix later *)
      intros.
      assert ({r < n} + {n <= r})...
        (* decidable *) admit. 
    ...
    assert ((sub M (S m)) == (sub (mu u) (S m)) ∪ ((sub M (S m)) ∩ (sub P (S m)))) as Mcond. 
      ...
      rewrite Empty_set_ident_left.
      fold (MinusPlus M).
      assert (sub (mu u) (S m) == sub (MinusPlus M) (S m)).
        rewrite sub_mu.
        rewrite mu'_MinusPlus.
        rewrite <- sub_mu. 
        rewrite sub_MinusPlus. 
        assert (sub M (S (S m)) == sub (mu u) (S (S m))).
          apply maxprop2...
        rewrite H5...
        admit. (* ?? *)
      rewrite H5.
      ...
      apply (H10 x)...
        ...
        apply In_Intersection...
        unfold MinusPlus in H11; apply In_Intersection in H11...
        apply all_decidable... 
        apply cellcond.
    ...
    assert ((sub P (S m)) == (sub (pi u) (S m)) ∪ ((sub M (S m)) ∩ (sub P (S m)))) as Pcond. 
      ...
      rewrite Empty_set_ident_left.
      fold (PlusMinus P).
      assert (sub (pi u) (S m) == sub (PlusMinus P) (S m)).
        rewrite sub_pi.
        rewrite pi'_PlusMinus.
        rewrite <- sub_pi. 
        rewrite sub_PlusMinus. 
        assert (sub P (S (S m)) == sub (pi u) (S (S m))).
          apply maxprop2...
        rewrite H7...
        admit. (* ?? *)
      rewrite H7.
      ...
      apply (H10 x)...
        apply In_Intersection in H9...
        apply In_Intersection...
        unfold PlusMinus in H11; apply In_Intersection in H11...
        apply all_decidable... 
        apply cellcond.

    assert (Disjoint (sub (mu u) (S m)) (sub M (S m) ∩ sub P (S m))) as JJ. 
      admit. (* ?? *)
    assert (Disjoint (sub (pi u) (S m)) (sub M (S m) ∩ sub P (S m))) as KK'. 
      admit. (* ?? *)
    ...
    assert (Inhabited ((sub M (S m)) ∩ (sub P (S m)))) as MPInhab.
      ...
      apply maxprop1...
      unfold Same_pair. 
      rewrite Mcond. rewrite Pcond at 2. 
      rewrite H0. 
      rewrite Empty_set_ident_right.
      rewrite Empty_set_ident_right.
      intuition. 
        ...
        * apply well_formed_by_dimension.
          ...
          pose (lt_eq_eq_lt_dec k m)...
            ...
            apply (well_formed_Included (minus y))...
              apply Setminus_Included.
            constructor...
            unfold Setminus in H8...
            apply In_Intersection in H8...
            unfold In at 1 in H9...
            unfold In at 1 in H10...
            assert (In (Minus M) x0)...
              exists y...
            unfold Y, In at 1 in H4...
            apply triangle_rest_in_set in H4...
            apply sub_Included in H13...
            assert (Disjoint P (Minus M))...
              apply Prop_2_1_dual...
              apply cellcond.
              exists M...
            ...
            apply (well_formed_Included (sub P k)). 
            apply (well_formed_sub)... 
            apply cellcond. 
            apply Setminus_Included... 
