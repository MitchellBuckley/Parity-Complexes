...
Ltac misc :=
  ...
  Lemma qwerqwer : forall x y, In (plus y) x -> In (minus y) x -> False.
  ...
  Hint Extern 2 (False) =>
    ...
    | H : In (plus ?y) ?x, K : In (minus ?y) ?x |- False => apply (qwerqwer _ _ H K); assumption
    end.
...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
 ...
 Lemma cell_dim_n_property : 
  ...
  Lemma gggg {U : Type} : forall n X Z, cardinal X n -> cardinal Z n -> @Included U X Z -> X == Z.
  Proof with intuition. (* careful, this is close to the axiom of choice *)
    admit.
  Qed.
  ...
  Proof with intuition.    
    ...
    eapply (H k). rewrite H6; left... admit. assert (W ⊆ sub Full_set n0). admit. exact H7. 
  ...
  Proof with intuition. 
    ...
    assert ( MinusPlus (sub M (S n)) ⊆ sub M n). 
      ...
      unfold In at 1 in H4...
      apply minus_dim in H5. rewrite <- H5 in H8. inversion H8... 
    ...
    assert (Inhabited (minus w))... 
      apply minus_Inhabited. subsuptac... rewrite H8...
  ...
  Proof with intuition. 
    ...
    unfold Same_set, Included...
      ...
      unfold is_a_cell in H... apply H2...
        apply H0. apply J. apply H0. apply J.
      rewrite H1... 
...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  ...
  Proof with intuition.
    ...
    unfold Y. 
    assert ((sup M (n - 1) ∪ ((sub M n ∪ Minus X) ∩ √Plus X)) == (((M ∪ Minus X) ∩ √Plus X))) as JA. admit.
    assert ((sup P (n - 1) ∪ ((sub M n ∪ Minus X) ∩ √Plus X)) == (((P ∪ Minus X) ∩ √Plus X))) as JB. admit.
    rewrite JA, JB in *.
    unfold moves_def in *...
    ...
    unfold Same_set, Included; repeat (basic; intuition).  
      ...
      right. repeat (basic; intuition). apply In_Complement... (* dim contradiction *) admit.
      right. apply In_Intersection... (* dim contradiction *) admit.
      apply In_Complement... (* contradiction H5 *) admit.
      apply In_Complement... (* dim contradiction *) admit.
      left. (* in sub M n, so in sub P n *) admit.
 ...
 Qed.
  ...
  Proof with intuition.
    ...
    assert (is_initial_segment S' (sub M (S n))) as S'initial. 
      unfold is_initial_segment. split. unfold S', Included...
...
(*
  ...
  *)  
    ...
    rewrite Ycond. apply (Inhabited_Included (minus x)). 
      apply minus_Inhabited. rewrite xdim. admit. 
  ...
  Lemma wertwert {U : Type} : forall X n, @cardinal U X (S n) -> Inhabited X.
  Proof with intuition.
    intros.
    remember (S n) as K.
    induction H.
  ...
  Proof with intuition. 
    ...
    assert (PlusMinus X == Singleton z). 
      ...
      assert (Inhabited (PlusMinus X)). 
        apply (asdfasdf _ 0)... apply wertwert in H...
...
(* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ *)
  ...
  Proof with intuition.
    ...
    assert (exists m, (In Z m /\ forall r, In Z r -> r <= m)) as max_exists.
      assert (Inhabited Z /\ Finite Z) as Inhab_Finite.
        split. 
        admit.
        apply (Finite_Included'' (λ m : nat, m < n))... 
        apply lt_n_is_Finite.
        unfold Z, Included, In... admit.
...
(*    set (Z := (fun m => (m >= n) \/ ((sub M (S m), sub P (S m)) === (sub (mu u) (S m), sub (pi u) (S m))))). 
    assert (exists m, (In Z m /\ forall r, In Z r -> r >= m)) as min_exists. 
      admit.
    inversion min_exists as [m H]; clear min_exists; inversion H as [minZ mmin]; clear H. 
    *)
    assert ((sub M (S m)) == (sub (mu u) (S m)) ∪ ((sub M (S m)) ∩ (sub P (S m)))) as Mcond. admit. (*
      unfold Z, In in minZ. 
      inversion minZ as [minZa | minZb]; clear minZ. 
      (* both empty*)  admit. 
      unfold Same_pair in minZb...
      unfold Same_set, Included; repeat (basic; intuition).*)
    ...
    assert ((sub P (S m)) == (sub (pi u) (S m)) ∪ ((sub M (S m)) ∩ (sub P (S m)))) as Pcond. 
      ...
      assert ((sub M (S m), sub P (S m)) === (sub (mu u) (S m), sub (pi u) (S m))). 
        admit. (* by maximality, might need to redefine this thing as the minimum with the right behaviour *)
      inversion H; clear H.
        unfold Same_pair in H1...
        unfold Same_set, Included; repeat (basic; intuition).
      ...
      split. 
        split.
          ...
          assert (triangle_rest X x1 x). 
            eapply (tr_trans _ _ x)... admit. 
            unfold less. exists x0...
            apply (tr_refl)...
          assert (x = x1). 
          apply (TTT x1)... admit. admit.
    ...
    assert (~(x ∈ (sub (mu u) (S m))) \/ ~(y ∈ (sub (mu u) (S m)))) as ASD. 
      assert (~((x ∈ (sub (mu u) (S m))) /\ (y ∈ (sub (mu u) (S m))))) as Q.
        unfold not... admit.
      assert ((x ∈ sub (mu u) (S m)) \/ ~(x ∈ sub (mu u) (S m))) as DD. admit.
    ...
    inversion xory; clear xory.
      ...
      split.
        ...
        apply Intersection_Included_cancel_right... admit.
        apply Intersection_Included_cancel_right... admit.
    ...
    ∪ ((sub (sup M (m - 1) ∪ sub P m) m ∪ Minus (Singleton y))
       ∩ √Plus (Singleton y))) ∪ Singleton y) (sup P m ∪ Singleton y))... 
        ...
        split. 
          unfold not; intros.
          unfold N, Q in H2. 
          (* show M is not {y}, presumably if that were the case, then m isn't minimal? *) admit.
        split. 
          unfold not; intros.
          unfold L, R in H2. 
          unfold celldim, setdim in H2. assert (dim y <= m). apply H2... rewrite ydim in H3...
        ...
        unfold Same_set, Included; repeat (basic; intuition). 
        assert ( dim x0 <= m \/ dim x0 > m)... admit.
        right; left...
        assert ( (x0 = y) \/ ~(x0 = y))... admit.
        left. repeat (basic; intuition)... left... 
        (* hmmm... not sure *) admit.
        admit. admit.
        (* hmmm... not sure *) admit.
        exfalso. assert (dim x0 = m). admit. admit.
        unfold sup, In at 1 in H8... 
        inversion H8; clear H8. subst.
